...
function detectLanguage(editor) {
    ...
    return cfg.get("defaultLanguage") ?? "python";
}
// ---------------------------------------------------------------------------
// Result panel
// ---------------------------------------------------------------------------
let resultPanel;
function showResultPanel(context, result, task) {
    if (!resultPanel) {
        resultPanel = vscode.window.createWebviewPanel("edgecoderResult", "EdgeCoder Result", vscode.ViewColumn.Beside, { enableScripts: true });
    ...
    const cfg = vscode.workspace.getConfiguration("edgecoder");
    const showRoute = cfg.get("showRouteInfo") !== false;
...
// Core run function
// ---------------------------------------------------------------------------
async function runEdgeCoder(context, endpoint) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
    ...
    const cfg = vscode.workspace.getConfiguration("edgecoder");
    const baseUrl = cfg.get("providerUrl") ?? "http://127.0.0.1:4304";
    const autoDetect = cfg.get("autoDetectLanguage") !== false;
    const language = autoDetect ? detectLanguage(editor) : (cfg.get("defaultLanguage") ?? "python");
    // Check server is up
    try {
        await getJson(`${baseUrl}/health`, 3000);
    }
    catch {
        const launch = await vscode.window.showErrorMessage("EdgeCoder agent is not running. Start it with: npm run dev:ide", "Copy command");
    ...
    await vscode.window.withProgress({
        ...
        try {
            const result = await postJson(`${baseUrl}${endpoint}`, { task, language, maxTokens: 1024 }, 90000);
...
function activate(context) {
    ...
    const poll = async () => {
        ...
        const baseUrl = cfg.get("providerUrl") ?? "http://127.0.0.1:4304";
        try {
            const s = await getJson(`${baseUrl}/status`, 3000);
    ...
    // Commands
    context.subscriptions.push(vscode.commands.registerCommand("edgecoder.runTask", () => runEdgeCoder(context, "/run")), vscode.commands.registerCommand("edgecoder.runLocal", () => runEdgeCoder(context, "/run/local")), vscode.commands.registerCommand("edgecoder.runSwarm", () => runEdgeCoder(context, "/run/swarm")), vscode.commands.registerCommand("edgecoder.applyCode", async () => {
        if (!resultPanel) {
